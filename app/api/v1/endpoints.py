...
@router.post(
    ...
    responses={400: {"model": ErrorResponse}, 500: {"model": ErrorResponse}},
    summary="Update an existing diagram structure using a free-text instruction",
    tags=["generation"],
)
async def update_diagram_structure(
    payload: UpdateByPromptInput = Body(...),
) -> ProcessStructureInput:
    """
    Apply a free-text update instruction to the current canvas structure.

    The endpoint receives the current ProcessStructureInput (canvas state as
    returned by the frontend getStructure() call) together with a natural-language
    instruction describing what to change (e.g. 'Add a new step after X',
    'Remove the decision about Y', 'Rename actor Z to W').

    The LLM returns a modified structure which is sanitized and validated against
    the ProcessStructureInput schema before being returned to the frontend.
    The frontend is then responsible for calling setStructure() on the canvas
    and — if needed — triggering the standard PlantUML generation flow separately.

    This endpoint does NOT generate PlantUML and does NOT write to the database.
    """
    current_dict = payload.current_structure.model_dump()

    try:
        updated_dict = update_structure_by_prompt(
            current_structure=current_dict,
            update_instruction=payload.update_instruction,
        )
    except Exception as exc:
        raise HTTPException(
            status_code=500,
            detail=f"LLM structure update failed: {exc}",
        ) from exc

    if not updated_dict.get("actors") or not updated_dict.get("actions"):
        raise HTTPException(
            status_code=500,
            detail="LLM returned an empty or incomplete structure. Please try again.",
        )

    try:
        updated_structure = _build_process_structure_from_structured_dict(updated_dict)
    except Exception as exc:
        raise HTTPException(
            status_code=400,
            detail=f"Updated structure failed validation: {exc}",
        ) from exc
